...
const getQuizCountsByCourses = async (
  courseIds,
  gradeId,
  termId,
  subjectId,
  ...
  const quizCounts = await Quiz.findAll({
    attributes: [
      ...
      [
        Sequelize.fn(
          "COUNT",
          Sequelize.fn("DISTINCT", Sequelize.col("quiz_id")),
        ),
        "quiz_count",
      ],
    ],
  ...
  quizCounts.forEach((item) => {
    quizCountMap[item.course_id] = Number(item.quiz_count);
  });
  return quizCountMap;
};
...
const getTopics = async (req, res, next) => {
  ...
  try {
    ...
    const quizCountMap = await getQuizCountsByCourses(
      courseIds,
      gredId,
      termId,
      subjectId,
    ...
    data.forEach((topic) => {
      topic.courses?.forEach((course) => {
        course.setDataValue("quiz_count", quizCountMap[course.course_id] || 0);
      });
    });
    if (data.length === 0) {
      res.status(204).send({ message: "No Content" });
